...
if (FormFormat > 0) then
	' Insert the form
	OutBuf(OUTPUT_FORM_SEARCHBOX) = OutBuf(OUTPUT_FORM_SEARCHBOX) & STR_FORM_SEARCHFOR & " <input type=""text"" name=""zoom_query"" size=""20"" value=""" & htmlspecialchars(query) & """ id=""zoom_searchbox"" class=""zoom_searchbox"" />" & VbCrlf
	...
	if (FormFormat = 2) then
		...
		next
		OutBuf(OUTPUT_FORM_RESULTSPERPAGE) = OutBuf(OUTPUT_FORM_RESULTSPERPAGE) & "</select><br /><br /></span>" & VbCrlf
		...
		if (UseCats = 1) then
			...
			if (SearchMultiCats = 1) then
				...
				next
				OutBuf(OUTPUT_FORM_CATEGORIES) = OutBuf(OUTPUT_FORM_CATEGORIES) & "</ul><br /><br />" & VbCrlf
			else
		...
		OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<br /><br /></span>" & VbCrlf
	else
...
if (NoSearch = 0) then
	...
	OutBuf(OUTPUT_HEADING) = OutBuf(OUTPUT_HEADING) & "<br /><br /></div>" & VbCrLf
						...
						elseif (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_MULTI) then
							...
							if (metavalues(zoomit)(fieldnum)(0) > 0) then
								...
								for mqi = 0 to num_multi_query
									for mvi = 0 to metavalues(zoomit)(fieldnum)(0)
	...
	if (IsMaxLimitExceeded = 1) then
		OutBuf(OUTPUT_SUMMARY) = OutBuf(OUTPUT_SUMMARY) & STR_PHRASE_CONTAINS_COMMON_WORDS & "<br /><br />"
	end if
	...
	if (num_pages > 1) then
		OutBuf(OUTPUT_PAGESCOUNT) = OutBuf(OUTPUT_PAGESCOUNT) & "<div class=""result_pagescount""><br />" & num_pages & " " & STR_PAGES_OF_RESULTS & "</div>" & VbCrlf
	end if
			...
			for fieldnum = 0 to NumMetaFields-1
				...
				if (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_MULTI) then
					if (metavalues(ipage)(fieldnum)(0) > 0) then
						...
						for mvi = 1 to metavalues(ipage)(fieldnum)(0)
							if (mvi > 1) then
								OutResBuf = OutResBuf & ", "
							end if
	...
	if (num_pages > 1) then
		...
		OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & "<div class=""result_pages"">" & STR_RESULT_PAGES & " "
		if (zoompage > 1) then
	...
	if (Timing = 1 OR Logging = 1) then
		...
		if (Timing = 1) then
			OutBuf(OUTPUT_SEARCHTIME) = OutBuf(OUTPUT_SEARCHTIME) & "<div class=""searchtime""><br /><br />" &  STR_SEARCH_TOOK & " " & ElapsedTime & " " & STR_SECONDS & "</div>"
		end if
	end if
...
if (ZoomInfo = 1) then
	OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & VbCrLf & "<center><p><small>" & STR_POWEREDBY & " <a href=""http://www.wrensoft.com/zoom/"" target=""_blank""><b>Zoom Search Engine</b></a></small></p></center>"
end if
